...
def _need(key: str, msg: str = "Complete the previous step first.") -> None:
    """Stop rendering and show a warning if a required session key is absent."""
...
def step_eda() -> None:
        ...
        if len(num_cols) >= 2:
            ...
            if high:
                st.markdown(
                    ...
                    f'{len(high)} pair(s) with |r| > 0.8. Consider removing one from each pair.</div>',
                    unsafe_allow_html=True,
                )
                st.dataframe(
            ...
            explain(
                ...
                "Features with |r| > 0.8 carry redundant information.",
                kind="learn",
            )
        else:
    ...
    with tab_out:
        ...
        if num_cols:
            ...
            explain(
                ...
                "KMeans is sensitive to them; DBSCAN handles them natively.",
                kind="learn",
            )
    ...
    with tab_stats:
        ...
        explain(
            ...
            "Large std relative to mean = feature may need scaling.",
            kind="learn",
        )
...
def step_clean() -> None:
    ...
    if miss_df.empty:
        st.markdown(
            '<div class="success-box"><strong>✓ No missing values!</strong></div>',
            unsafe_allow_html=True,
        )
    else:
    ...
    if auto_remove:
        ...
        st.markdown(
            ...
            f'{safe_names} — likely IDs or constant values.</div>',
            unsafe_allow_html=True,
        )
...
def step_features() -> None:
    ...
    if low_var:
        ...
        st.markdown(
            f'<div class="warn-box"><strong>⚠ Low Variance:</strong> '
            f'{safe_lv} — near-constant, may add noise.</div>',
            unsafe_allow_html=True,
        )
    ...
    with mode_tab:
        ...
        with c1:
            st.markdown(
                ...
                f'<div style="font-family:IBM Plex Mono;font-size:0.6rem;letter-spacing:0.12em;'
                f'text-transform:uppercase;color:#fbbf24;margin-bottom:0.4rem">{_safe_html(info["level"])}</div>'
                f'<div style="font-size:0.82rem;color:#6b7090;line-height:1.6">{info["desc"]}</div>'
                f'</div>',
                unsafe_allow_html=True,
            )
        with c2:
            st.markdown(
                ...
                f'<div style="font-family:IBM Plex Mono;font-size:0.6rem;color:#34d399;'
                f'letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.3rem">✓ Best when</div>'
                f'<div style="font-size:0.82rem;color:#6b7090">{info["best"]}</div>'
                f'</div>'
                f'<div>'
                f'<div style="font-family:IBM Plex Mono;font-size:0.6rem;color:#fb7185;'
                f'letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.3rem">✗ Avoid when</div>'
                f'<div style="font-size:0.82rem;color:#6b7090">{info["worst"]}</div>'
                f'</div></div>',
                unsafe_allow_html=True,
            )
...
def _run_automl(df: pd.DataFrame, scaler: str, reduction: str, n_km: int) -> None:
    ...
    for i, (name, cfg) in enumerate(configs):
        ...
        status.markdown(
            f'<div style="font-family:IBM Plex Mono;font-size:0.7rem;color:#6b7090;">'
            f'▸ Testing {_safe_html(name)} {_safe_html(cfg)}…</div>',
            unsafe_allow_html=True,
        )
        try:
    ...
    st.markdown(
        ...
        f'Silhouette = <strong>{best["Silhouette ↑"]}</strong> '
...
def step_results() -> None:
    ...
    if metrics.get("Noise pts", 0) > 0:
        st.markdown(
            ...
            f'(DBSCAN label -1) — excluded from scoring.</div>',
            unsafe_allow_html=True,
        )
    ...
    with tab_elbow:
        ...
        explain(
            ...
            "<strong>Silhouette sweep</strong>: pick the peak.",
            kind="learn",
        )
...
def _glossary() -> None:
    ...
    for i, (term, defn) in enumerate(terms.items()):
        ...
        with col:
            st.markdown(
                f'<div style="background:#111225;border:1px solid #1e2035;border-radius:6px;'
                f'padding:0.7rem 0.9rem;margin-bottom:0.6rem;">'
                f'<div style="font-family:IBM Plex Mono;font-size:0.72rem;color:#22d3ee;margin-bottom:0.2rem">'
                f'{_safe_html(term)}</div>'
                f'<div style="font-size:0.8rem;color:#6b7090;line-height:1.5">{_safe_html(defn)}</div>'
                f'</div>',
                unsafe_allow_html=True,
            )
